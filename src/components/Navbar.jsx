...
const Navbar = iconTray => {
  ...
  return (
    <div className="NavbarSetup h-[15vh] w-full p-5 ">
      <div className="protector h-full w-full">
        <nav className="h-full w-full flex flex-wrap justify-between items-start">
          <div className="logo px-10 py-5">
            <svg
              width="100"
              height="32"
              viewBox="0 0 121 32"
          ...
          {iconTray.iconEnabled === "true"
            ? <div className="buttonGroup flex gap-5 items-center h-full px-10">
                {/* onClick={handleRedirect} */}
                <button className="btn2 border-[1px] border-black/50 rounded-full p-5 px-6 capitalize font-medium bg-white ">
                  <Link to={"/register"}>get projects</Link>
                </button>
                <button className="btn border-[1px] border-black rounded-full p-5 px-6 capitalize font-medium bg-black text-white ">
                  onboard talent
                </button>
              </div>
            : iconTray.iconEnabled === "false"
              ? <div className="crossIcon h-[5rem] w-[5rem] mr-28 cursor-pointer flex items-center justify-center border-[1px] border-black/60 rounded-full">
                  <Link to={"/"}>
                    <svg
                      width="40"
                      height="40"
